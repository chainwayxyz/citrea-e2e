...
impl BridgeBackendConfig {
    ...
    pub fn get_env(&self) -> HashMap<String, String> {
        ...
        env.insert("HOST".to_string(), self.client.host.clone());
        env.insert("PORT".to_string(), self.client.port.to_string());
        env.insert(
            "CORS_ORIGIN_PATTERN".to_string(),
            "^http://localhost ".to_string(),
        );

        env.insert("PGHOST".to_string(), self.client.pghost.clone());
        env.insert("PGPORT".to_string(), self.client.pgport.to_string());
        env.insert("PGUSER".to_string(), self.client.pguser.clone());
        env.insert("PGPASSWORD".to_string(), self.client.pgpassword.clone());
        env.insert("PGDATABASE".to_string(), self.client.pgdatabase.clone());
        env.insert("PGSSLMODE".to_string(), "prefer".to_string());

        env.insert("REDIS_URL".to_string(), self.client.redis_url.clone());

        env.insert("NETWORK".to_string(), "regtest".to_string());
        env.insert(
            "USER_TAKES_AFTER".to_string(),
            self.client.user_takes_after.clone(),
        );
        env.insert("VERIFIER_PKS".to_string(), self.client.verifier_pks.clone());
        env.insert(
            "BRIDGE_AMOUNT_BTC".to_string(),
            self.client.bridge_amount_btc.to_string(),
        );
        env.insert(
            "CONFIRMATION_THRESHOLD".to_string(),
            self.client.confirmation_threshold.to_string(),
        );

        env.insert(
            "BITCOIN_REST_ENDPOINT".to_string(),
            self.client.bitcoin_rest_endpoint.clone(),
        );
        env.insert(
            "BITCOIN_REST_AUTH_HEADER".to_string(),
            self.client.bitcoin_rest_auth_header.clone(),
        );

        env.insert(
            "CITREA_REST_ENDPOINT".to_string(),
            self.client.citrea_rest_endpoint.clone(),
        );
        env.insert(
            "BITCOIN_LIGHTCLIENT_CONTRACT_ADDRESS".to_string(),
            self.client.bitcoin_lightclient_contract_address.clone(),
        );
        env.insert(
            "BRIDGE_CONTRACT_ADDRESS".to_string(),
            self.client.bridge_contract_address.clone(),
        );
        env.insert(
            "DECLARE_WITHDRAW_FILLER_PRIVATE_KEY".to_string(),
            self.client.declare_withdraw_filler_private_key.clone(),
        );

        env.insert(
            "FAUCET_PRIVATE_KEY".to_string(),
            self.client.faucet_private_key.clone(),
        );
        env.insert(
            "FAUCET_AMOUNT".to_string(),
            self.client.faucet_amount.clone(),
        );
        env.insert(
            "FAUCET_AMOUNT_LIMIT".to_string(),
            self.client.faucet_amount_limit.clone(),
        );
        env.insert(
            "FAUCET_COUNT_LIMIT".to_string(),
            self.client.faucet_count_limit.clone(),
        );

        env.insert(
            "OPERATOR_URLS".to_string(),
            self.client.operator_urls.clone(),
        );
        env.insert(
            "VERIFIER_URLS".to_string(),
            self.client.verifier_urls.clone(),
        );
        env.insert(
            "AGGREGATOR_URL".to_string(),
            self.client.aggregator_url.clone(),
        );
