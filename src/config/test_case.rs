...
pub struct TestCaseConfig {
    ...
    /// Path to Clementine directory.
    /// Expected to have `certs` directory with `ca`, `aggregator`, `client`, `server` subdirectories.
    /// Expected to have regtest_paramset.toml file.
    /// Defaults to `resources/clementine` if not set.
