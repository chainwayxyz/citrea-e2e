...
cat > $CERT_DIR/openssl_x509.cnf << EOF
[req]
...
openssl req -new -x509 -sha256 -days 365 -key $CA_DIR/ca.key -out $CA_DIR/ca.pem \
  ...
  -extensions v3_ca -config $CERT_DIR/openssl_x509.cnf
...
openssl req -new -key $SERVER_DIR/server.key -out $SERVER_DIR/server.csr \
  ...
  -config $CERT_DIR/openssl_x509.cnf
...
openssl x509 -req -sha256 -days 365 -in $SERVER_DIR/server.csr \
  ...
  -extfile $CERT_DIR/openssl_x509.cnf -extensions v3_req
...
rm -f $CERT_DIR/openssl_x509.cnf
